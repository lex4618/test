...
from testcase.common.aklibbrowser import libbrowser
import time
import traceback
...
class AkubelaPanelWebV2:
    ...
    def __init__(self):
        ...
        self.web_admin_pwd = 'Ak#123456'
        self.new_pwd = 'Ak#123456'
        self._rom_version = ''
        self.device_ip = ''
        self.login_url = ''
        self.ele_info = None
    ...
    def init_without_start(self, browser=None):
        ...
        if not self.version_branch:
            self.ele_info = self.device_config.get_device_normal_web_element_info('PANELWEB2_0')
        else:
            self.ele_info = self.device_config.get_device_web_element_info(self.version_branch, 'PANELWEB2_0')
    ...
    def is_opened_browser(self):
        aklog_info('is_opened_browser')
        try:
            if self.browser is not None and self.browser.get_driver() is not None:
                aklog_info('browser is already opened')
                return True
            else:
                aklog_info('browser is not opened')
                return False
        except:
    ...
    def get_device_config_by_version(self, firmware_version):
        aklog_debug('get_device_config_by_version, %s' % firmware_version)
        if firmware_version:
    ...
    def restore_device_config(self):
        aklog_debug('restore_device_config')
    ...
    def click_cancel(self):
        ...
        for i in range(2):
            if self.browser.is_exist_and_visible_ele_by_xpath(self.ele_info['page_cancel_xpath']):
                aklog_debug('click cancel success')
                self.browser.restore_implicitly_wait()
                return True
            elif i == 0:
                if self.browser.is_exist_alert():
                ...
                aklog_error('click cancel failed')
                self.browser.screen_shot()
                self.browser.restore_implicitly_wait()
                return False
    ...
    def read_config(self, id_name_xpath, timeout=None):
        ...
        try:
            ...
            aklog_debug('value: %s, Type: %s' % (value, type(value)))
            return value
        except:
            aklog_error(traceback.format_exc())
            return None
    ...
    def get_selected_option_by_xpath(self, ele_xpath):
        ...
        aklog_debug('get_selected_option_by_xpath  ele_xpath: %s' % ele_xpath)
    ...
    def get_select_options_list_by_xpath(self, ele_xpath):
        ...
        aklog_debug('get_select_options_list_by_xpath  ele_xpath: %s' % ele_xpath)
    ...
    def delete_table_all(self, title=None):
        ...
        if self.browser.get_ele_status_by_xpath(delete_all_btn_xpath):
            ...
            aklog_info('Delete All不可点击，列表应该已被全部删除')
    ...
    def judge_input_int_range(self, id_name_xpath, write_data, input_min, input_max, return_tips=False):
        ...
        if write_data.isdigit() and int(write_data) in range(int(input_min), int(input_max) + 1):
            ...
            if not ret:
                aklog_error('元素: %s 输入了 %s 保存失败!' % (id_name_xpath, write_data))
                self.click_cancel()
            return ret == write_data
        else:
            ret = self.get_submit_result(id_name_xpath)
            if ret is True:
                aklog_info('元素: %s 输入了 %s, 不在有效范围内, 没有报错!'
    ...
    def judge_input_submit_result(self, id_name_xpath, write_data, limit_length=None, return_tips=False):
        ...
        if submit_result is True:
            ...
            if input_value == write_data:
                ...
                aklog_error('保存结果跟输入不一致，或密码保存位数不正确')
                self.screen_shot()
                return False
        else:
            aklog_error('保存失败，提示语：%s' % submit_result)
            if return_tips:
                # 返回提示语可以用来测试提交失败的情况
    ...
    def input_date_by_xpath(self, ele_xpath, date_str):
        ...
        aklog_info('input_date_by_xpath, ele_xpath: %s, date: %s' % (ele_xpath, date_str))
    ...
    def input_time_by_xpath(self, ele_xpath, time_str):
        ...
        aklog_info('input_time_by_xpath, ele_xpath: %s, time: %s' % (ele_xpath, time_str))
            ...
            for i in range(4):
                if self.browser.is_exist_and_visible_ele_by_xpath(self.ele_info['login_username_xpath']):
                    ...
                    if submit_tips == 'Incorrect password or username!':
                        aklog_warn('%s:%s 登录用户名和密码错误，更换密码重新登录' % (
                        self.web_admin_username, self.web_admin_pwd))
                        if self.web_admin_pwd != 'admin':
                            self.web_admin_pwd = 'admin'
                        elif self.web_admin_pwd != self.new_pwd:
                            self.web_admin_pwd = self.new_pwd
                        continue
    ...
    def login_by_hc(self):
        """先登录家庭中心，然后再切换到网关设备网页"""
        if self.browser.is_exist_and_visible_ele_by_xpath('//*[@akubela-test="login-login"]'):
            self.write_config('//*[@akubela-test="login-name"]', self.web_admin_username)
            self.write_config('//*[@akubela-test="login-passward"]', self.web_admin_pwd)
            self.click_btn_by_xpath('//*[@akubela-test="login-login"]')
            time.sleep(3)
            self.browser.click_btn_by_xpath(
                '//span[text()="%s"]/../../..//span[text()="Setting"]' % self.device_ip)
            time.sleep(2)
            self.browser.close_window()
            self.browser.switch_last_window()
            ...
            if self.browser.is_exist_and_visible_ele_by_xpath(self.ele_info['account_sip_status_xpath']):
                for j in range(0, 60):
                    ...
                    if account_register_status:
                        aklog_info('account_register_status: %s' % account_register_status)
                        return account_register_status
                    else:
                        time.sleep(5)
                        self.browser.web_refresh()
            elif i == 0:
                aklog_error('获取账号注册状态失败，可能是页面异常，重试...')
                self.browser.screen_shot()
                self.login()
                self.enter_account_basic()
                time.sleep(5)
        aklog_error('获取账号注册状态失败')
        self.browser.screen_shot()
        return None
    ...
    def wait_for_account_to_register_successfully(self, failure_to_wait_time=15):
        ...
        """
        aklog_info('wait_for_account_to_register_successfully')
        self.enter_account_basic()
        i = 0
        while i < 30 + int(failure_to_wait_time / 5):
            ...
            if account_status == 'Registered':
                ...
                i += 1
                continue
        aklog_info('sip account register failed')
        return False
    ...
    def web_basic_upgrade(self, firmware_path, accept=True, reset=False):
        ...
        aklog_info('web_basic_upgrade, firmware_path: %s' % firmware_path)
        ...
        if accept:
            ...
            # self.enter_settings_basic()
            if self.browser.is_exist_and_visible_ele_by_xpath(self.ele_info['upgrade_basic_firmware_version_xpath']):
                aklog_info('返回升级基础页面成功')
                return 1
            else:
                aklog_error('返回升级基础页面失败')
                return 0
        time.sleep(30)
        # 判断是否处于升级状态
    ...
    def web_basic_upgrade_to_version(self, dst_version, firmware_path, accept=True, reset=False):
        """网页基础升级"""
        aklog_info('web_basic_upgrade_to_version, dst_version: %s, firmware_path: %s'
    ...
    def upgrade_failed_reboot(self):
        ...
        for i in range(0, 2):
            if self.browser.is_exist_and_visible_ele_by_id('tUpgrade'):
                ...
                aklog_info('重启后没有正常刷新到基础升级页面，需要重新加载')
                self.browser.screen_shot()
                self.login()
                self.enter_settings_basic()
        aklog_error('重启失败，请检查原因')
        self.browser.screen_shot()
        return False
    ...
    def copy_old_firmware_to_download_dir(self):
        ...
        aklog_info('copy_old_firmware_to_download_dir')
        self.device_config.get_old_firmware(force_replace=self.force_replace_old_version)  # 强制替换旧版本升级包
    ...
    def set_network_to_dhcp_after_reset(self):
        aklog_info('set_network_to_dhcp_after_reset')
        if self.device_config.get_lan_port_type() == 'static':
            ...
            aklog_info('network type is already dhcp')
    ...
    def web_reset_to_factory_setting(self, accept=True, boot_time_after_get_ip=None):
        ...
        if not ret:
            aklog_error('恢复出厂，设备未重启或重启失败，请检查')
            self.browser.screen_shot()
            return False
        ...
        if self.browser.is_exist_and_visible_ele_by_xpath(self.ele_info['upgrade_basic_factory_default_xpath']) \
            ...
            aklog_info('恢复出厂设置后没有正常刷新到基础升级页面，需要重新加载')
            self.browser.screen_shot()
            wait_time = round(time.time() - begin_time)
    ...
    def web_reset_to_installer_setting(self, accept=True, retry_login=True):
        ...
        if not ret:
            aklog_error('恢复到installer，设备未重启或重启失败，请检查')
            self.browser.screen_shot()
            return False
        ...
        if self.browser.is_exist_and_visible_ele_by_xpath(self.ele_info['upgrade_basic_reset_to_installer_xpath']) \
                or self.browser.is_exist_and_visible_ele_by_xpath(self.ele_info['login_username_xpath']):
            aklog_info('恢复到installer设置完成')
        else:
            aklog_info('恢复到installer设置后没有正常刷新到基础升级页面，需要重新加载')
            self.browser.screen_shot()
            wait_time = round(time.time() - begin_time)
            if wait_time < reset_default_time:
                continue_wait_time = reset_default_time - wait_time
        ...
        # 判断是否返回基础升级页面
        if self.browser.is_exist_and_visible_ele_by_xpath(self.ele_info['upgrade_basic_reset_to_installer_xpath']):
            aklog_info('恢复到installer设置成功')
            return True
        else:
            aklog_error('恢复到installer设置失败，请检查原因')
            self.browser.screen_shot()
            return False
    ...
    def rename_all_cfg_file(self):
        aklog_info('rename_all_cfg_file')
    ...
    def enter_hide_page_8848(self):
        ...
        aklog_info('打开页面: %s' % url)
        for i in range(2):
            self.browser.visit_url(url)
            if self.browser.get_value_by_xpath(self.ele_info['hidden_page_title_xpath']) == '8848':
                return True
            elif i == 0:
                self.login()
        aklog_error('进入8848页面失败')
        return False

    def set_ssh_or_tln(self, option_value):
        """option_value 取值0和1"""
        aklog_info('set_ssh_or_tln %s' % option_value)
    ...
    def web_open_ssh(self, disable_first=False, wait_time=10):
        """设备网页打开的ssh操作"""
        aklog_info()
    ...
    def export_system_log(self):
        ...
        for i in range(0, 20):
            if not os.path.exists(export_system_log_file):
                aklog_info('system log文件导出中...')
                time.sleep(3)
            else:
                aklog_info('system log文件导出成功')
                time.sleep(3)
                return True
        aklog_error('system log文件导出导出失败')
        self.browser.screen_shot()
        return False
    ...
    def export_pcap(self):
        ...
        for i in range(0, 20):
            if not os.path.exists(export_pcap):
                aklog_info('pcap文件导出中...')
                time.sleep(3)
            else:
                aklog_info('导出pcap文件成功')
                time.sleep(3)
                return True
        aklog_error('pcap文件导出导出失败')
        self.browser.screen_shot()
        return False
    ...
    def exec_command_by_interactive_ssh_thread(self, command):
        ...
        aklog_info('exec_command_by_interactive_ssh_thread, command: %s' % command)
        for i in range(2):
    ...
    def exec_command_by_interactive_telnet_thread(self, command):
        ...
        aklog_info('exec_command_by_interactive_telnet_thread, command: %s' % command)
        for i in range(2):
            if self.tln.is_connected() or self.tln.command_stop():
                break
            elif i == 0:
    ...
    def check_device_process_status(self):
        ...
        aklog_info('check_device_process_status')
    ...
    def export_syslog_to_result(self, case_name):
        ...
        aklog_info(f'export_syslog_to_result, export_syslog_type: {export_syslog_type}, case_name: {case_name}')
